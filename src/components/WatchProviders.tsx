...
export function WatchProviders({ providers }: WatchProvidersProps) {
  ...
  const currentRegion = providers[selectedRegion] ? selectedRegion : availableRegions[0];
  const data = providers[currentRegion];

  if (!data) return null;

  const hasProviders = (data.flatrate?.length ?? 0) > 0 || (data.rent?.length ?? 0) > 0 || (data.buy?.length ?? 0) > 0;
  ...
  if (!hasProviders) {
      return (
        <div className="space-y-4">
             ...
             <p className="text-muted-foreground text-sm">No streaming information available for this region.</p>
        </div>
      )
  }
